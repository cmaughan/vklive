...
fs::path file_documents_path()
{
    // Temporary fix until I figure out how to properly write to folders/find them on Mac!!
#if 1
    return home_path();
#else
...
fs::path file_roaming_path()
{
    ...
    LOG(ERROR, "Search path not found");
    return home_path() / "Documents" / "vklive";
#endif
}
...
/*
    ...
    auto files = file_gather_files(m_root);
    for (auto& file : files)
    {
        if (file.extension() == ".janet" || 
            file.extension() == ".ixi")
        {
            auto asset = std::dynamic_pointer_cast<OpusAsset>(std::make_shared<MusicFileAsset>(this, file));
            Register(asset);
        }
    }
...
bool Opus::SaveTo(const fs::path& folder)
{
    try
    {
        fs::create_directories(folder);
        fs::copy(m_root, folder, fs::copy_options::recursive | fs::copy_options::overwrite_existing);
        return true;
    }
    catch (std::exception&)
    {
        // UI Message
        UIManager::Instance().AddMessage(MessageType::Error | MessageType::System, "Couldn't save Opus!");
        return false;
    }
}
*/
