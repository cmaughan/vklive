...
static uint64_t vknvg_compareCreatePipelineKey(const VKNVGCreatePipelineKey* a, const VKNVGCreatePipelineKey* b)
{
    if (a->renderPass != b->renderPass)
    {
        return uint64_t(a->renderPass) - uint64_t(b->renderPass);
    }
...
static void vknvg_bindPipeline(VKNVGcontext* vk, VkCommandBuffer cmdBuffer, VKNVGCreatePipelineKey* pipelinekey)
{
    VKNVGPipeline* pipeline = vknvg_findPipeline(vk, pipelinekey);
    if (!pipeline)
    {
        // For now, the renderpass has to match, so always create?
...
#include <vklive/vulkan/nanovg_shaders/fill.vert.inc>
    };

    const uint32_t fillFragShader[] = {
#include <vklive/vulkan/nanovg_shaders/fill.frag.inc>
    };
    const uint32_t fillFragShaderAA[] = {
#include <vklive/vulkan/nanovg_shaders/fill_edge_aa.frag.inc>
    };
...
static void vknvg_renderFlush(VulkanContext& ctx, void* uptr)
{
    VKNVGcontext* vk = (VKNVGcontext*)uptr;
    VkDevice device = vk->createInfo.device;
    uint32_t currentFrame = *vk->createInfo.currentFrame;
    ...
    if (vk->ncalls > 0)
    {
        debug_begin_region(vk->createInfo.cmdBuffer, "nanovg_flush", glm::vec4(1.0f));
        ...
        debug_end_region(vk->createInfo.cmdBuffer);
    }
    // Reset calls
    vk->nverts = 0;
    vk->npaths = 0;
