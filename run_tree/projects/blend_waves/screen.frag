...
layout(set = 1, binding = 1) uniform sampler2D default_color;
...
void main()
{
    ...
    float wave = texture( AudioAnalysis, vec2(uv.x * .25, 0.75) ).x;
    vec3 waveColor = vec3(0.001, 0.01, 0.04) / abs(wave - uv.y + 0.66);

    float wave2 = texture( AudioAnalysis, vec2(uv.x * .25, 0.55) ).x;
    vec3 waveColor2 = vec3(0.04, 0.01, 0.001) / abs(wave - uv.y + 0.366);

    ledColor += waveColor + waveColor2;

    uv = gl_FragCoord.xy / ubo.iResolution.xy;
    vec4 last_color = texture( default_color, uv );
    float frac = 0.05;
    fragColor = (vec4(ledColor, 1.0) * frac) + (last_color * (1.0f - frac));
